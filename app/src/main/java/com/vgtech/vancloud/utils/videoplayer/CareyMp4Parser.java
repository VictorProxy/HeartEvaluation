...
public class CareyMp4Parser {
	...
	public CareyMp4Parser(IsoFile isoFile) {
		...
		Log.i("test", "timescale : " + timescale);
		
		lengthInSeconds = duration * 1.0 / timescale;
		
		Log.i("test", "lengthInSeconds : " + lengthInSeconds);
		...
		SampleToChunkBox sampleToChunkBox = trackBox.getSampleTableBox()
				.getSampleToChunkBox();
		Log.i("test", "sampleToChunkBox : " + sampleToChunkBox);
		...
		if (sampleToChunkBox != null
			...
			if (sampleSizeBox.getSampleSize() > 0) {
				Log.i("test", "sampleSizeBox : " + sampleSizeBox.getSampleSize());
			...
			if (trackBox.getParent().getBoxes(MovieExtendsBox.class).size() > 0) {
				...
				for (MovieFragmentBox movieFragmentBox : trackBox
	...
	public void printinfo() {
		Log.i("test", "视频总时长(秒): " + lengthInSeconds);
		Log.i("test", "关键帧 \t 帧偏移 \t 帧大小 \t 帧对应的时间");

		int size = syncSamples.length;

		for (int i = 0; i < size; i++) {
			Log.i("test", syncSamples[i] + " " + syncSamplesOffset[i]
